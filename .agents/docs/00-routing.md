...
When behavior changes, update production code, its owner document, relevant verification, and the router only if request selection changed. A compatible route or implementation within current policy does not need an ADR; boundary and persistence-policy changes may.
