...
Android Room receives an application-owned database path, BundledSQLite, and `Dispatchers.IO`. The manifest sets `android:allowBackup="false"`. The route-only snapshot capability uses private `SharedPreferences`: its small initial read is synchronous during composition, while writes and cleanup are serialized as background `commit()` operations and awaited. This store does not persist screen state. Android Navigation 3 saved state separately covers Activity recreation.
