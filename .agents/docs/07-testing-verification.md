...
Changes to the Maestro runner, Xcode preflight, or version helper require the shell contract check from the [command catalog](01-stack-toolchain.md#commands). The normal shared gate runs it on macOS and Unix hosts with Bash; native Windows skips it with an explicit reason and does not establish shell-runner coverage. The check uses stubs and no real Xcode installation or device.
