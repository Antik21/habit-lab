...
- Before individual iOS Gradle, build, or test commands outside that runner, use the repository-owned [`xcode-preflight.sh`](../../../../../ui-tests/maestro/xcode-preflight.sh) as a toolchain gate. Its standalone invocation gives a concise readiness result. For a command sequence, source it once in the current shell, call its preflight function, and use its checked wrappers so every operation inherits the pinned `DEVELOPER_DIR` and exact `xcodebuild` identity without parsing stdout. The helper validates the selected toolchain against its single numeric Xcode baseline. The [toolchain catalog](../../../../docs/01-stack-toolchain.md) remains the source of general compatibility and toolchain context; the contributor Xcode need not numerically equal the exact CI pin. If preflight or applicable local compatibility requirements fail, report `blocked` before running any iOS command.
