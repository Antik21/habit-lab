...
Path prechecks alone are insufficient because another process can replace a path between validation and use. Create run directories, write artifacts, and clean scratch with race-safe filesystem operations appropriate to the action: no-follow resolution, directory-handle-anchored relative operations, and exclusive creation where ownership or uniqueness is claimed. Fail closed if identity, type, containment, or ownership changes during an operation. Executable adapters and stress tests for this contract belong to DEN-17/DEN-19 and are not part of this scaffold.
