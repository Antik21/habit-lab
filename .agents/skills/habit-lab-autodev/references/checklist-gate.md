...
Before a new pass/fail event is appended, every existing attempt and reread artifact is revalidated by hash and structured content. Once `finish` writes the terminal record, the run is immutable: `add`, `freeze`, `pass`, `fail`, and another `finish` are rejected before writes; only read-only `status` remains available.
