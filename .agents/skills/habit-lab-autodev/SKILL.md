...
- Treat the repository Maestro runner as the current UI verification baseline, not as a generic AutoDev adapter. Do not manufacture missing adapters, reservation pools, executable gates, or self-patching mechanisms. Reviewed, skill-owned Maestro subflows are the limited exception: they compose only through the runner-owned common scenario and do not change its runner/configuration contract.
