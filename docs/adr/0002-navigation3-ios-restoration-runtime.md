...
its matching experiment caller. Corrupt JSON, an unknown version, stale IDs, impossible ordering,
or an overlong stack is cleared and replaced with Gallery before Navigation 3 sees it.
...
with experiment, nested-flow, and dialog routes before release.
